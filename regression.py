...
svr_train_X_st, svr_train_y_st = get_input(df_train, 'finish_time', True)
svr_model_st = SVR(kernel='linear',C=0.1,epsilon=0.1)
svr_model_st.fit(svr_train_X_st,svr_train_y_st)
...
gbr_train_X_st, gbr_train_y_st = get_input(df_train, 'finish_time', True)
params = {'n_estimators': 100, 'max_depth': 6, 'learning_rate': 0.1, 'loss': 'huber'}
gbrt_model_st = GradientBoostingRegressor(**params)
gbrt_model_st.fit(gbr_train_X_st, gbr_train_y_st)

# Gridsearch
parameters = {'loss':('ls', 'lad', 'huber', 'quantile'), 'max_depth':[2,3,4,5,6], 'learning_rate':[0.01,0.05,0.1,0.5],'n_estimators':[25,50,75,100]}
clf = GridSearchCV(GradientBoostingRegressor(), parameters, cv=10, refit="neg_mean_squared_error", n_jobs=4, scoring="neg_mean_squared_error" )
clf.fit(svr_train_X_st, svr_train_y_st)
print(clf.best_estimator_)
clt_test_X_st, clt_test_y_st = get_input(df_test, 'finish_time', True)
clt_pred_y_st = clf.best_estimator_.predict(clt_test_X_st)
print(mean_squared_error(clt_test_y_st, clt_pred_y_st))


parameters = {'kernel':('linear', 'poly', 'rbf', 'sigmoid'), 'C':[0.1, 1, 10, 100], 'epsilon':[0,0.01,0.1,0.5,1,2]}
clf = GridSearchCV(SVR(), parameters, cv=10, refit="neg_mean_squared_error", n_jobs=4, scoring="neg_mean_squared_error" )
clf.fit(svr_train_X_st, svr_train_y_st)
print(clf.best_estimator_)
clt_test_X_st, clt_test_y_st = get_input(df_test, 'finish_time', True)
clt_pred_y_st = clf.best_estimator_.predict(clt_test_X_st)
...
# Predicting on Test Data: Root Mean Squared Error(RMSE), Top_1, Top_3 and Average_Rank
# use the mean and variance of the TRAINING DATA to normalize the prediction result
'''
...
print(mean_squared_error(svr_test_y, svr_pred_y))
...
print(mean_squared_error(gbrt_test_y, gbrt_pred_y))
'''
# RMSE with normalization of SVR
svr_test_X_st, svr_test_y_st = get_input(df_test, 'finish_time', True)
svr_pred_y_st = svr_model_st.predict(svr_test_X_st)
print(mean_squared_error(svr_test_y_st, svr_pred_y_st))

# RMSE with normalization of GBRT
gbrt_test_X_st, gbrt_test_y_st = get_input(df_test, 'finish_time', True)
gbrt_pred_y_st = gbrt_model_st.predict(gbrt_test_X_st)
print(mean_squared_error(gbrt_test_y_st, gbrt_pred_y_st))
...
def top_1(method):
    if method == "SVR":
        print("Top_1 of SVR")
        df_test['top_1_pred'] = svr_pred_y_st
    else:
        print("Top_1 of GBRT")
        df_test['top_1_pred'] = gbrt_pred_y_st
    top_1_correct = 0
